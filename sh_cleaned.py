...
def prep_image(image, subtract_median=True, rotation_angle=-59, crop_xmin=0, crop_xmax=350, crop_ymin=30, crop_ymax=380):
    '''
    prepare an image for later analysis by subtracting bg, cropping, and rotating so grid is vertical
    '''
    if subtract_median:
        sub = image - np.median(image)
        crop = sub[crop_xmin:crop_xmax, crop_ymin:crop_ymax]
        rotate = scipy.ndimage.rotate(crop, rotation_angle)
        return rotate
    else:
        crop = image[crop_xmin:crop_xmax, crop_ymin:crop_ymax]
        rotate = scipy.ndimage.rotate(crop, rotation_angle)
        return rotate
    
    

def offset_dist_to_tilt(offset_dist, return_mrad=False, f_1=200, f_2=97.158, f_3=26.831):
    '''
    Convert an offset as measured on the detector to tilt wavefront error on the PTT array
    
    Arguments:
    offset_dist - the offset distance on the detector (in the same units as the focal lengths, default mm)
    return_mrad (default False)- boolean, do you want the answer in radians (False) or mrad (True)
    f_1 (default 200 [mm]) focal length of the focusing lens outside the SH
    f_2 (default 97.158 [mm]) focal length of the collimating lens inside the SH
    f_3 (default 26.831 [mm]) focal length of the lenslets in the lenslet array inside the SH
    '''
    theta_2 = np.arctan(offset_dist / f_3)
    d1 = f_2 * np.tan(theta_2)
    theta_1 = np.arctan(d1/f_1)
    if return_mrad:
        return theta_1*1e3
    else:
        return theta_1
...
def centroid(imag, x, y, boxsize, thresh):
    ...
    x
    y
    boxsize - size of box to use
    thresh - threshold value
    
    Outputs
    ---------
    int(x-boxsize/2)+cenx
    int(y-boxsize/2)+ceny
    counts
    '''
    ibx = int(boxsize)
...
def shspacing(shimage,axis):
    '''
    Computes mean spacing of spots in an axis
    
    Inputs
    ---------
    shimage - SHWFS image to use 
    axis - axis along which to compute spacing
...
class SH:
    def __init__(self, img, pixscale=1, flux_frac = 0.5, centroiding_threshold=0, background=None, two_mirrors=False, pupil_radius_max = 1.0):
        '''
    Create instance of SH class
    
    Arguments
    ---------
    img - image to analyze (allowable types: np.ndarray, str where the str represents the filename of a fits image)
        --> self.img attribute
    pixscale (default:1) - pixel scale 
        --> self.pixscale attribute
    flux_frac (default:0.5) - fraction of maximum flux required to count a spot as a spot
        --> self.flux_frac attribute
    centroiding_threshold (default:0)- threshold for centroid computation
        --> self.centroiding_threshold attribute
    background (default:None)- background image to subtract
        --> self.background attribute
    two_mirrors (default:False) - boolean denoting whether the image contains two mirrors or not
        --> self.dual attribute
    pupil_radius_max (default:1.0) - maximum pupil radius value
        --> self.pupil_radius_max attribute'''
    ...
    def computegridofcentroids(self):
        ...
        Attributes Created
        -------------------
        self.fluxmap - 2D array of fluxes at each spot posn
        self.xcenmap - 2D array of x centroid positions
        self.ycenmap - 2D array of y centroid positions
        self.xrefmap - 2D array of x reference positions
        self.yrefmap - 2D array of y reference positions
        '''
        xmin = self.offsets[0]
        ymin = self.offsets[1]
        img = self.img
...
#                 print('spacingx', spacingx)
    ...
    def process(self, nzern = 10, x1=None, y1=None, x2=None, y2=None):
        '''
        Process the SH image - includes LSQ fitting for zernike coefficients
        If two_mirrors=True when initialized, must include points (x1, y1) and (x2, y2) that define the line separating the two mirrors.
        
        Arguments
        ------------
        nzern (int, default:10) - number of zernike terms to compute and fit
        x1 (default: None) - x coordinate of first point on line separating mirrors
        y1 (default: None) - y coordinate of first point on line separating mirrors
        x2 (default: None) - x coordinate of second point on line separating mirrors
        y2 (default: None) - y coordinate of second point on line separating mirrors
        
        New Attributes
        --------------
        self.zxy - the zerndx and zerndy arrays stacked horizontally (2D array of shape nzern (or nzern+3 if two_mirrors=True) x  nspots)
        self.zcoeffs - the value of the zernike coefficients computed by least squares fit
        self.xcentroids - the measured positions of the spots (x coords)
        self.ycentroids - the measured positions of the spots (y coords)
        self.rms
        self.rms_ttf
        self.xyrefs
        self.xrefs
        ...
        self.xrefs_ttf
        self.yrefs_ttf
        self.xvec - the vector between each xcentroid point and each xrefs_ttf point
        self.yvec - the vector between each ycentroid point and each yrefs_ttf point
        self.xyrefs_ho - higher order xy refs
        self.xrefs_ho - only the xrefs (higher order)
        self.yrefs_ho - only the yrefs (higher order)
        self.xvec_ho - the vector between each xcentroid point and each xrefs_ho point
        self.yvec_ho - the vector between each ycentroid point and each yrefs_ho point
        self.norms 
        self.m1 - an array telling which spots are on mirror 1
        self.m2 - an array telling which spots are on mirror 2
            e.g. self.xcentroids[self.m2.astype(bool)] will return only the xcentroids values that belong to spots on mirror2
        '''
        halist = []
        declist = []
        comaxlist = []
        ...
        if self.dual:
            ...
            nsp0 = self.mirror1_spots_to_use.shape[0]
            nsp1 = self.mirror1_spots_to_use.shape[1]
        ...
        for j in range(nz): # creating each row of the matrix
            ...
            if j == 0 and self.dual:
                ...
                dupdzdx = np.reshape(duplicate_dzdx, (nsp0,nsp1))   
                dupdzdy = np.reshape(duplicate_dzdy, (nsp0,nsp1))
        ...
        if self.dual:
            xtilt = self.zcoeffs[1] #duplicated xtilt
            ytilt = self.zcoeffs[3] #duplicated ytilt
            self.difference_pixels = np.sqrt(xtilt**2 + ytilt**2)

    def get_offsets_twomirrors(self):
        '''
        Uses the zcoeffs values to compute the x and y tilt differences
        
        new attributes:
        self.xTiltDifference (value in radians)
        self.yTiltDifference (value in radians)
        self.total_difference (value in radians)
        '''
        pixelsize = 16 # microns per pixel
        xtilt = self.zcoeffs[1]
        ytilt = self.zcoeffs[3]
        diff_mm = self.difference_pixels * pixelsize / 1e3
        xtilt_mm = xtilt * pixelsize / 1e3
        ytilt_mm = ytilt * pixelsize / 1e3
        xtilt_radians = offset_dist_to_tilt(xtilt_mm)
        ytilt_radians = offset_dist_to_tilt(ytilt_mm)
        diff_radians = offset_dist_to_tilt(diff_mm)
        self.xTiltDifference = xtilt_radians
        self.yTiltDifference = ytilt_radians
        self.total_difference = diff_radians
        
        
    def make_twomirror_vectorplot(self, vector_scale=10, plot_title='SH Image', savefig=False, savename='filename.png'):
        m1_xcentroids = self.xcentroids[self.m1.astype(bool)]
        m1_ycentroids = self.ycentroids[self.m1.astype(bool)]
        m2_xcentroids = self.xcentroids[self.m2.astype(bool)]
        m2_ycentroids = self.ycentroids[self.m2.astype(bool)]

        m1_xvec = self.xvec[self.m1.astype(bool)]
        m1_yvec = self.yvec[self.m1.astype(bool)]
        m2_xvec = self.xvec[self.m2.astype(bool)]
        m2_yvec = self.yvec[self.m2.astype(bool)]

        pl.figure(figsize=(10,10))
        pl.imshow(self.img, origin='lower', cmap='gray')
        pl.title(plot_title)
        pl.scatter(self.xrefs_ttf, self.yrefs_ttf, color='green', label='ref')
        pl.scatter(m1_xcentroids, m1_ycentroids, color='blue', label='mirror1', alpha=0.5)
        pl.scatter(m2_xcentroids, m2_ycentroids, color='orange', label='mirror2', alpha=0.5)

        for i,x in enumerate(self.xrefs_ttf):
            pl.arrow(x, self.yrefs_ttf[i], vector_scale*self.xvec[i], vector_scale*self.yvec[i], color='white')
